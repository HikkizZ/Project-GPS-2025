...
import type { CreateInventoryExitData, InventoryExitDetailData } from "@/types/inventory/inventory.types"
...
const InventoryExitForm: React.FC<InventoryExitFormProps> = ({ onSubmit, onCancel, isSubmitting = false }) => {
    ...
    if (exitDate) {
      try {
        ...
        if (!isNaN(dateObj.getTime())) {
          ;(dataToSubmit as any).exitDate = dateObj.toISOString()
        } else {
          console.warn("Invalid exitDate provided, omitting from submission:", exitDate)
        }
      } catch (error) {
      ...
      <Row>
        <Col md={6}>
          <Form.Group className="mb-3" controlId="customerRut">
            ...
            <Form.Control.Feedback type="invalid">{errors.customerRut}</Form.Control.Feedback>
          </Form.Group>
        </Col>
        <Col md={6}>
          <Form.Group className="mb-3" controlId="exitDate">
            <Form.Label>Fecha de Salida (Opcional)</Form.Label>
            <Form.Control type="date" name="exitDate" value={exitDate} onChange={handleChange} />
          </Form.Group>
        </Col>
      </Row>
