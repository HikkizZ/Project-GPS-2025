...
const InventoryHistoryTable: React.FC<InventoryHistoryTableProps> = ({
  ...
  const totalPages = exportMode
    ? 1
    : Math.ceil(filteredAndSortedMovements.length / ITEMS_PER_PAGE);
  const startIndex = exportMode ? 0 : (currentPage - 1) * ITEMS_PER_PAGE;
  const endIndex = exportMode
    ? filteredAndSortedMovements.length
    : startIndex + ITEMS_PER_PAGE;
  const currentMovements = filteredAndSortedMovements.slice(
    startIndex,
    endIndex
  );
  ...
  }, [movementFilter, sortOrder, exportMode]);
        ...
        <Card.Body>
          ...
          <div
            className={
              exportMode
                ? "mb-4 text-center"
                : "d-none d-print-block mb-4 text-center"
            }
          >
            ...
            <p className="text-muted">
              Total de movimientos: {getFilteredCount()}
            </p>
          </div>

          {/* Controles de filtro y ordenamiento - ocultos en modo exportación */}
          {!exportMode && (
            <div className="d-flex justify-content-between align-items-center mb-3">
              <h6 className="mb-0">
                ...
                {movementFilter === "exit" && "Ventas Registradas"} (
                {getFilteredCount()} de {allEntriesCount})
              </h6>
              ...
              {!exportMode && (
                <div className="d-flex justify-content-between align-items-center mb-2">
                  <small className="text-muted">
                    ...
                    {Math.min(endIndex, filteredAndSortedMovements.length)} de{" "}
              ...
              <div className={exportMode ? "" : "table-responsive"}>
                <Table hover size={exportMode ? "sm" : undefined}>
                  <thead className="table-light">
                    <tr>
                      <th style={{ width: exportMode ? "200px" : "auto" }}>
                        Tipo de Movimiento
                      </th>
                      <th style={{ width: exportMode ? "150px" : "auto" }}>
                        Fecha
                      </th>
                      <th style={{ width: exportMode ? "250px" : "auto" }}>
                        Proveedor/Cliente
                      </th>
                      <th style={{ width: exportMode ? "150px" : "auto" }}>
                        Total
                      </th>
                      {!exportMode && <th className="text-center">Acciones</th>}
                    </tr>
                  </thead>
                  <tbody>
