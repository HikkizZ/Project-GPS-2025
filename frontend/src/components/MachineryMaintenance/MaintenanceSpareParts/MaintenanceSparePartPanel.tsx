...
const MaintenanceSparePartPanel: React.FC<Props> = ({ mantencionId, show, onHide, onReload }) => {
  ...
  const { create, loading: creating, reset: resetCreate } = useCreateMaintenanceSparePart();
  ...
  const handleAgregar = async () => {
    ...
    reload();
    reloadSpareParts();
    await onReload(); // ✅ actualización de la página principal
    resetCreate();
  };
  ...
  return (
    <Modal show={show} onHide={handleClose} size="lg">
      ...
      <Modal.Body>
        ...
        <h3 style={{ textAlign: 'center', minWidth: '250px', fontWeight: 700 }}>
          ...
          <Table striped bordered hover>
            <thead>
              <tr>
                ...
                {isSuperAdmin && (
                  <th style={{ textAlign: "center", minWidth: "250px", fontWeight: 700 }}>
                    Editar
                  </th>
                )}
              </tr>
      ...
      <EditMaintenanceSparePartModal
        ...
        stockDisponible={
          spareParts.find((s) => s.id === selectedItem?.repuesto?.id)?.stock ?? 0
        }
        onReload={reload}
      />
    </Modal>
